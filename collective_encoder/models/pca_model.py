...
from collective_encoder.models.modules.pca import PCAModule
...
class PCAModel(CEModelBase):
    ...
    def __init__(self, args: Dict[str, Any] = None, **kwargs) -> None:
        ...
        assert self.dataset_type in self._COMPATIBLE_DATASETS, (
            f"Dataset type '{self.dataset_type}' is not compatible with PCAModel. "
    ...
    def fit(self, datamodule=None, X: Optional[torch.Tensor] = None) -> "PCAModel":
        ...
        self.log_msg(
            f"PCAModel fitted successfully. Total explained variance ratio: "
            f"{float(torch.sum(self.encoder_net.explained_variance_ratio)):.4f}"
        )
        return self
