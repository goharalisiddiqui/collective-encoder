...
def colvardatset_args():
    desc = "Colvar Dataset Arguments"
    parser = argparse.ArgumentParser(description=desc)


    parser.add_argument('--colvarfile', type=str, help='Input file for training')
    parser.add_argument('--column_match', type=str, default=None, help='Only columns containing this string will be used')
    parser.add_argument('--labels', nargs='+', help='Label columns in the data file')
...
class LITColvarData(pl.LightningDataModule):
    def __init__(self,
        ...
        print("==========================================")
        print(f"Loading data from file: {colvarfile}")
        if not os.path.exists(colvarfile):
            raise FileNotFoundError(f"File {colvarfile} not found")
        with open(colvarfile) as f:
        ...
        self.alldata = np.loadtxt(colvarfile, usecols=cols_to_read)
        self.alllabel = np.loadtxt(colvarfile, usecols=[a for a in range(1,len(label_list) + 1)], ndmin = 2)
