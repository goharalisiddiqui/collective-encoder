...
def plot_H(fig, ax, train_x, train_y, val_x, val_y, model, label, i):
    ax.set_title("VAE Latent-space-"+str(i))
...
def plot_cv_frames(frames, data, cv_pos = 0):
    fig, ax = plt.subplots(1, 1, figsize=(13, 4))
    ax.set_title("VAR Generated frames and training data agianst CV at pos "+str(cv_pos))


    ax.plot(
        np.arange(1, frames.shape[0] + 1),
        np.asarray([x.cpu().detach().numpy()[cv_pos] for x in frames]),
        "+",
        c="tab:green",
        label="samples from model",
    )

    ax.plot(
        np.arange(1, data.shape[0] + 1),
        np.asarray([x[cv_pos] for x in data]),
        ".",
        c="tab:red",
        label="training data",
    )

    ax.set_ylabel(header[cv_pos])
    ax.set_xlabel("Frame No")

    ax.legend()
    fig.savefig(f"{outname}_samples.png", dpi=150)
    plt.close()
